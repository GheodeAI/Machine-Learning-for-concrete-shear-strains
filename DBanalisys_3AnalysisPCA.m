...
% dtTrn = xlsread('ANN_fortraining2.xlsx');
dtTrn = xlsread('ANN_fortraining_cro.xlsx');
Xtrn  = dtTrn(:,1:end-2); Y1trn  = dtTrn(:,end-1); Y2trn  = dtTrn(:,end);
% dtTst = xlsread('ANN_forpredicting.xlsx');
dtTst = xlsread('ANN_forpredicting.xlsx');
Xtst  = dtTst(:,1:end);

%%% Reduction
rho = corr(Xtrn)
figure; heatmap(round(100*rho));
% figure; corrplot(Xtrn)
%%%
[rho,pval1] = corr(Xtrn,Y1trn);
figure; bar(abs(rho)); xlabel('Input Variables'); ylabel('\kappa-correlation'); axis on; grid on;
figure; bar(pval1); xlabel('Snow variables'); ylabel('p-valor'); axis on; grid on;
[rho,pval1] = corr(Xtrn,Y2trn);
figure; bar(abs(rho)); xlabel('Input Variables'); ylabel('\epsilon-correlation'); axis on; grid on;
figure; bar(pval1); xlabel('Snow variables'); ylabel('p-valor'); axis on; grid on;
%%%
...
figure;bar(cumsum(explained));ylabel('Cumulative Explained Variance');xlabel('components');
axis([1 20 90 100])
figure;
...
zlabel('3rd P. Comp.')
% set(gca,'YScale','log')
[coeff,score,latent,tsquared,explained,mu] = pca(XPCA,'NumComponents',nKs);
lmdX = (XPCA-mu)*coeff; nT = size(XPCA,1);
Xm = XPCA-mean(XPCA); [U,S,V] = svd(Xm);
...
Xtrn = lmdX2(1:42,:);
Xtst = lmdX2(43:end,:);
% save('PCA1.mat','Xtrn','Xtst','Y1trn','Y2trn');
% return;
%%%
% Training;
rng(1);
...
writematrix(Yprd_rf,'RF_pred3PCA_cro.xls');
...
writematrix(Yprd_lr,'LREG_pred3PCA_cro.xls');
writematrix(Yprd_ls,'LASSO_pred3PCA_cro.xls');
...
writematrix(Yprd_g,'SVR_g_pred3PCA_cro.xls');
writematrix(Yprd_l,'SVR_l_pred3PCA_cro.xls');
writematrix(Yprd_p,'SVR_p_pred3PCA_cro.xls');
...
writematrix(Yprd,'MLP_pred3PCA_cro.xls');
