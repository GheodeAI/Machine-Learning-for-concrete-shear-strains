...
dtTst = xlsread('ANN_forpredicting2.xlsx');
Xtst  = dtTst(:,1:end);
...
[coeff,score,latent,tsquared,explained,mu] = pca(Xtrn);
...
zlabel('3rd P. Comp.')
% set(gca,'YScale','log')
[coeff,score,latent,tsquared,explained,mu] = pca(Xtrn,'NumComponents',nKs);
lmdX = (Xtrn-mu)*coeff;
Xm = Xtrn-mean(Xtrn); [U,S,V] = svd(Xm);
...
writematrix(Yprd_rf,'RF_pred3.xls');
...
writematrix(Yprd_lr,'LREG_pred3.xls');
writematrix(Yprd_ls,'LASSO_pred3.xls');
...
writematrix(Yprd_g,'SVR_g_pred3.xls');
writematrix(Yprd_l,'SVR_l_pred3.xls');
writematrix(Yprd_p,'SVR_p_pred3.xls');
...
writematrix(Yprd,'MLP_pred3.xls');
