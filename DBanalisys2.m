...
dt = xlsread('ANN_database_training');
rng(1);
prTst = 0.2; [m,n]=size(dt);
idxTst = randperm(round(prTst*m));
idxTrn = setdiff(1:m,idxTst);
Xtrn  = dt(idxTrn,1:end-2); Y1trn  = dt(idxTrn,end-1); Y2trn  = dt(idxTrn,end);
Xtst  = dt(idxTst,1:end-2); Y1tst  = dt(idxTst,end-1); Y2tst  = dt(idxTst,end);
...
R2_txx(k) = sqrt(mean((ypred_mlp_tst-Ytst).^2))
end
